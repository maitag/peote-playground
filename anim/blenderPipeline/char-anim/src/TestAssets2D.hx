...
class TestAssets2D extends Application
{
	...
	public function start(window:Window)
	{
		...
		program.setMultiTexture(PipelineTools.loadTextures(Pipeline2D.sheets), "custom");
		...
		for (tileID in TileID)
		{
			...
			var tile = Pipeline2D.tile(tileID);
			var sheet = Pipeline2D.sheets[tile.sheet];
