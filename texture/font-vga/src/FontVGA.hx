...
/** see haxe cookbook - https://code.haxe.org/category/other/vga-text-renderer.html **/
...
abstract FontVGA(Texture) to Texture {
	public function new() {
		...
		for (char_code in 0...char_count) {
			...
			var x = column * char_size;
			var y = row * char_size;
			var data_index = char_code << 3;

			for (char_y in 0...char_size) {
				var bits = data[data_index + char_y];
				for (char_x in 0...char_size) {
					var bit = (bits << char_x) & 0x80;
					var rgba = (bit == 0x80) ? 0xffffffFF : 0x00000000;
					texture_data.setColor(x + char_x, y + char_y, rgba);
				}
			}
