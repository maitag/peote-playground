...
class Bell implements Element
{
	...
	public static function init(world:World) {
		Bell.world = world;
		buffer = new Buffer<Bell>(1024, 1024, true);
	...
	public function new(color:Color, options:BodyOptions, audioBuffer:AudioBuffer)
	{
		this.color = color;
		...
		var variation = (Math.random() * 0.2) * polarity;
		pitch = 1 + variation;
		// audio = new AudioSource(audioBuffer);
	...
	{
		var s = new AudioSource(audioBuffer);
		s.pitch = pitch;
		s.play();		
	}
