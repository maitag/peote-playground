...
class TriangleDisplay extends Display 
{
	...
	public function new(x:Int, y:Int, width:Int, height:Int, color:Color=0x00000000, bufferSize:Int=1024) 
	{
		setDefaultBlendValues();

		// create empty buffer
		bufferBytes = BufferBytes.alloc(BufferElement.BUFF_SIZE * bufferSize);
		bufferBytes.fill(0, bufferBytes.length, 0);
		// TODO: for testing only create one at now

		super(x, y, width, height, color);
	}

	public function add(triangle:BufferElement) {
		triangle.bytePos = 0;
		triangle.writeBytes(bufferBytes);

		gl.bindBuffer(gl.ARRAY_BUFFER, glBuffer);
		gl.bufferSubData(gl.ARRAY_BUFFER, triangle.bytePos, BufferElement.BUFF_SIZE, new GLBufferPointer(bufferBytes));
	...
	override private function renderProgram(peoteView:PeoteView):Void
	{
		...
		gl.drawArrays( gl.TRIANGLES, 0, BufferElement.VERTEX_COUNT );
