...
class Main extends Application
{
	...
	public function startSample(window:Window)
	{
		...
		// init Bell Program and add it to Display
		Bell.init(world);
		Bell.addToDisplay(display);

		// init border at window edge
		Bell.borderWidth = window.width;
		Bell.borderHeight = window.height;
		
		var audioLoader = AudioBuffer.loadFromFile("assets/BELL_A_0.wav");
		audioLoader.onComplete(buffer -> {
			
			var blue = new Bell(Color.BLUE,
				{
					mass: 0, // static
					x: 400,
					y: 300,
			...
			window.onMouseDown.add((x, y, button) -> {
				...
				world.listen(red.body, blue.body, {
					...
					enter: (a, b, c) -> {
						var bell:Bell = a.data.element;
						bell.onCollide();
	
						var bell:Bell = b.data.element;
						bell.onCollide();
						trace("Collision Entered");
					}, // at first frame that a collision starts
