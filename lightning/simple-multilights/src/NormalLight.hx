...
class NormalLight implements Element
{
	...
	{	
		...
		"			
			vec4 normalLight( int colorTextureID, int normalTextureID, float depth )
			{
				//vec3 lightColor = vec3(1.0, 1.0, 1.0);
				vec3 lightColor = vColor0.rgb;
				...
				// fetch pixel from color-map
				vec4 colorTextureRGBA = getTextureColor( colorTextureID, (vPos + (vTexCoord - 0.5)*vSize)/texRes );
				
				// add the final attenuated light to the ambientColor
				vec3 intensity = diffuse * attenuation;
	
				//return normalTextureRGBZ; // to test the normal-mapping
				//return colorTextureRGBA; // to test the color-mapping
				return vec4(intensity*colorTextureRGBA.rgb, colorTextureRGBA.a);
			}			
		");
	...
	public function new(x:Int = 50, y:Int = 50, size:Int = 100, depth:Float = 0.1, color:Color)
	{
		this.x = x;
		this.y = y;
		this.size = size;
