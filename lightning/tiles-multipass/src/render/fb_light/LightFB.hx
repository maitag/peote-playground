	...
	{	
		...
		program.injectIntoFragmentShader(
			...
			vec4 normalLight( int normalDepthID, float depth )
			{
				...
				vec3 falloff = vec3(0.4, 0.4, 0.4); // TODO: in depend of size!
				...
				vec4 normalTextureRGBZ = getTextureColor( normalDepthID, ( vPos + vTexCoord*vSize )/texRes );
				
				
				// vector to position of light
				// vec3 light = vec3(  vTexCoord - 0.5 ,  normalTextureRGBZ.a - depth );
				// vec3 light = vec3(  vTexCoord - 0.5 ,  (0.25 + normalTextureRGBZ.a / 2.0 - depth) );
				// vec3 light = vec3( (vTexCoord-0.5) , (normalTextureRGBZ.a - depth)  );
				vec3 light = vec3( (vTexCoord-0.5) , (normalTextureRGBZ.a - depth + 1.0)  );
				
				
				// float D = length(light);
				float D = length(light)*0.5;

				// calculate the light falloff
				// float attenuation = 1.0 / ( falloff.x + (falloff.y * D) + (falloff.z * D * D) );
				// float attenuation = clamp(1.0 - D * D / (0.5 * 0.5), 0.0, 1.0);
				float attenuation = max(1.0 - D * D / (0.75), 0.0);
				attenuation *= attenuation;

				// normalize
				vec3 N = normalize(normalTextureRGBZ.rgb * 2.0 - 1.0);
				// vec3 N = normalTextureRGBZ.rgb * 2.0 - 1.0;
