...
abstract UvAoAlphaDepthFB(Display) to Display
{
	...
	{	
		...
		"	
			vec4 uvAoAlphaDepth( vec4 normalDepthTexture, vec4 uvAoAlphaTexture, int haxeUVTextureID)
			{
				// z-buffer
				if (normalDepthTexture.a < 1.0) gl_FragDepth = normalDepthTexture.a;
				...
				// test without UV-mapping
				// return vec4( vec3(1.0,1.0,1.0) * uvAoAlphaTexture.b, uvTex.a * uvAoAlphaTexture.a );
		...
		program.setColorFormula( "uvAoAlphaDepth(normalDepth, uvAoAlpha, haxeUV_ID)" );
